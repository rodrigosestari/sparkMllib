...
public final class Collect {
	...
	public static void main(String[] args) {
		SparkConf sparkConf = new SparkConf().setAppName("SparkMllibSql");
		JavaSparkContext sc = new JavaSparkContext(sparkConf);
		try {
			SQLContext sqlContext = new org.apache.spark.sql.SQLContext(sc);
			//@SuppressWarnings("unchecked")
			JavaRDD<Administration> admin =  sc.textFile("hdfs://quickstart.cloudera:8020//user/cloudera/resultpig/part-r-00000").map(
			...
			try {
				model_create = NaiveBayesModel.load(sc.sc() , "hdfs://quickstart.cloudera:8020//user/cloudera/models/NaiveBayes");
			} catch (Exception e) {
			...
			if (null == model_create){
				...
				model_create.save(sc.sc() , "hdfs://quickstart.cloudera:8020//user/cloudera/models/NaiveBayes");
			...
			clusters.save(sc.sc(), "hdfs://quickstart.cloudera:8020//user/cloudera/models/modelCluster"+Long.toString(time));
			...
			String directoryCluster = "hdfs://quickstart.cloudera:8020//user/cloudera/models/resultCluster/points"+Long.toString(time); 
			result.saveAsTextFile(directoryCluster);
			
			System.out.println("Sum Squared Errors = " + error);
			System.out.println("Directory Cluster:" + directoryCluster);
